package com.lojaapi.exceptionhandler;
...
@ControllerAdvice
public class LojaApiExceptionHandler extends ResponseEntityExceptionHandler{
	@Autowired
	...
	public ResponseEntity<Object> handleUnexpectedTypeException(UnexpectedTypeException ex, WebRequest request){
		String msgUsuario = message.getMessage("validacao.desconhecida", null, LocaleContextHolder.getLocale());
		String msgDesenvolvedor = ExceptionUtils.getRootCauseMessage(ex);
		List<Erro> errors = Arrays.asList(new Erro(msgUsuario, msgDesenvolvedor));
		return handleExceptionInternal(ex, errors, new HttpHeaders(), HttpStatus.BAD_REQUEST, request);
	}
